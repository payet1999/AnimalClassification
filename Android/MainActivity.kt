...
class MainActivity : AppCompatActivity() {
    ...
    private fun initViews() {
        // Hides yet unused image view created to display the taken picture and predicted animal
        findViewById<ImageView>(R.id.background_imageview).visibility = View.GONE
        findViewById<ImageView>(R.id.prediction_imageview).visibility = View.GONE
    ...
    /**
     * Updates the predicted animal image with the output result
     */
    private fun setPredictionImage() {
        // Get bitmap from image

        predictedDogIndex?.let { index ->
            var imageToSet = dogBreedsLabels[index].lowercase(Locale.ROOT).replace(" ", "_") + (0..5).random().toString()
            findViewById<ImageView>(R.id.prediction_imageview).setImageDrawable(
                resources.getDrawable(resources.getIdentifier(imageToSet, "drawable", packageName), theme)
            )
            findViewById<ImageView>(R.id.prediction_imageview).visibility = View.VISIBLE
            return
        }
        predictedAnimalIndex?.let { index ->
            var imageToSet = ANIMAL_LABELS[index].replaceFirstChar { it.lowercaseChar() } + (0..5).random().toString()
            findViewById<ImageView>(R.id.prediction_imageview).setImageDrawable(
                resources.getDrawable(resources.getIdentifier(imageToSet, "drawable", packageName), theme)
            )
            findViewById<ImageView>(R.id.prediction_imageview).visibility = View.VISIBLE
